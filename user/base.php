<?php
// This file should be included after session_start() and user authentication check
...
<head>
    ...
    <title><?= $page_title ?? 'NCITAD - User Portal' ?></title>
    ...
    <aside id="sidebar" class="sidebar-full w-72 bg-gradient-to-b from-slate-800 via-slate-900 to-slate-950 text-white flex flex-col shadow-2xl z-40 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
        ...
        <div class="mx-4 mb-6 bg-white/10 backdrop-blur-sm rounded-xl p-4 border border-white/10 flex-shrink-0">
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center text-xl font-bold shadow-lg">
                    <?= strtoupper(substr($user['username'] ?? 'U', 0, 1)) ?>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="font-semibold text-sm truncate"><?= htmlspecialchars($user['username'] ?? 'User') ?></p>
                    <p class="text-xs text-blue-200">Faculty</p>
                </div>
            </div>
        </div>
        ...
        <nav class="flex-1 px-4 space-y-1 overflow-y-auto pb-4 sidebar-scroll">
            ...
            <a href="concernlist.php" class="group flex items-center px-4 py-3.5 rounded-xl transition-all duration-200 <?= $current_page === 'concernlist.php' ? 'bg-gradient-to-r from-blue-500 to-blue-600 shadow-lg shadow-blue-500/50' : 'hover:bg-white/10' ?>">
                <i class="bi bi-file-earmark-text mr-3 text-lg <?= $current_page === 'concernlist.php' ? 'text-white' : 'text-blue-300 group-hover:text-white' ?>"></i>
                <span class="font-medium <?= $current_page === 'concernlist.php' ? 'text-white' : 'text-gray-300 group-hover:text-white' ?>">My Concerns</span>
                <?php if ($current_page === 'concernlist.php'): ?>
                    <i class="bi bi-chevron-right ml-auto text-white"></i>
                <?php endif; ?>
            </a>
            
            <a href="addnewconcern.php" class="group flex items-center px-4 py-3.5 rounded-xl transition-all duration-200 <?= $current_page === 'addnewconcern.php' ? 'bg-gradient-to-r from-blue-500 to-blue-600 shadow-lg shadow-blue-500/50' : 'hover:bg-white/10' ?>">
                <i class="bi bi-plus-circle mr-3 text-lg <?= $current_page === 'addnewconcern.php' ? 'text-white' : 'text-blue-300 group-hover:text-white' ?>"></i>
                <span class="font-medium <?= $current_page === 'addnewconcern.php' ? 'text-white' : 'text-gray-300 group-hover:text-white' ?>">Add New Concern</span>
                <?php if ($current_page === 'addnewconcern.php'): ?>
                    <i class="bi bi-chevron-right ml-auto text-white"></i>
                <?php endif; ?>
            </a>
            ...
            <a href="settings.php" class="group flex items-center px-4 py-3.5 rounded-xl transition-all duration-200 <?= $current_page === 'settings.php' ? 'bg-gradient-to-r from-blue-500 to-blue-600 shadow-lg shadow-blue-500/50' : 'hover:bg-white/10' ?>">
                ...
                <span class="font-medium <?= $current_page === 'settings.php' ? 'text-white' : 'text-gray-300 group-hover:text-white' ?>">Settings</span>
